...
class PointCloudParser(SensorMessageParser):
    ...
    def __init__(self, output_path, instance_saving=True, suffix='.pcd'):
        ...
        for t, s in zip(self._md['type'], self._md['size']):
            np_type = pypcd.pcd_type_to_numpy_type[(t, s)]
            self._typenames.append(np_type)
        self._np_dtype = np.dtype(list(zip(self._md['fields'], self._typenames)))

    def convert_xyzit_pb_to_array(self, xyz_i_t, data_type=None):
    ...
    def convert_array_to_xyzit_pb(self, arr):
        xyz_i_t=[]
        for point in arr:
            xyz_i_t.append(pypcd.PointXYZIT(point[0], point[1], point[2], point[3], point[4]*1e9))
        return xyz_i_t

    def convert_pc_to_xyzit(pb_pointcloud):
        """
        将 pypcd.PointCloud 对象转换为 PointXYZIT 对象列表
        """
        xyz_i_t_list = []  # 用于存储转换后的 PointXYZIT 对象

        # 从 PointCloud 对象提取数据
        pc_data = pb_pointcloud.pc_data  # 这是一个 numpy 数组，包含 x, y, z, intensity, timestamp 等字段

        for point in pc_data:
            # 创建一个新的 PointXYZIT 对象
            xyzit_point = pointcloud_pb2.PointXYZIT()
            xyzit_point.x = point['x']
            xyzit_point.y = point['y']
            xyzit_point.z = point['z']
            xyzit_point.intensity = point['intensity']
            xyzit_point.timestamp = int(point['timestamp'] * 1e9)  # 将时间戳转换回纳秒

            # 将创建的 PointXYZIT 对象添加到列表中
            xyz_i_t_list.append(xyzit_point)
    ...
    def make_xyzit_point_cloud(self, xyz_i_t):
        ...
        """
        pc_data = self.convert_xyzit_pb_to_array(xyz_i_t, data_type=self._np_dtype)
        self._md['width'] = len(xyz_i_t)
        self._md['points'] = len(xyz_i_t)
        pc = pypcd.PointCloud(self._md, pc_data)
        return pc
    def make_contidata_point_cloud(self,pc_data):
        return pypcd.PointCloud(self._md,pc_data)    
